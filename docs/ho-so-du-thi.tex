...
\begin{titlepage}
  \centering
  \vspace*{18mm}
  {\Large \textbf{HỒ SƠ DỰ ÁN}}\\[10mm]
  {\large CUỘC THI SÁNG TẠO TRẺ QUỐC GIA TRONG LĨNH VỰC TRÍ TUỆ NHÂN TẠO}\\[25mm]
  {\Huge\bfseries\color{navy}
  Hệ thống phát hiện nội dung độc hại Tiếng Việt chống né lọc,\\
...
\bottomrule
\endfoot
Nội dung độc hại & Nội dung được gán nhãn HATE trong bộ dữ liệu, bao gồm các phát ngôn công kích, thù ghét hoặc xúc phạm theo quy tắc gán nhãn của nguồn dữ liệu.\\
Né lọc & Việc biến đổi hình thức văn bản để làm giảm khả năng phát hiện của bộ lọc, trong khi vẫn giữ ý nghĩa chính của câu.\\
Teencode & Cách viết không chuẩn thường dùng trong môi trường trực tuyến, chẳng hạn thay thế chữ, số, ký hiệu hoặc cách viết tắt.\\
Ký tự đồng dạng & Ký tự có hình thức gần giống ký tự mục tiêu nhưng có thể thuộc bảng mã hoặc mã Unicode khác.\\
Điểm mù & Mẫu biến thể vẫn giữ nguyên sau khi đi qua bộ chuẩn hóa, được dùng để kiểm tra giới hạn của phương pháp dựa trên luật.\\
Chuẩn hóa & Quá trình đưa văn bản về dạng nhất quán hơn trước khi phân loại, gồm xử lý Unicode, ký tự đồng dạng, dấu phân cách, lặp ký tự và từ điển teencode.\\
Mô hình nền & Mô hình đã được huấn luyện trước, sau đó được tinh chỉnh cho bài toán cụ thể. Trong dự án, mô hình nền là ViHateT5.\\
Tinh chỉnh & Huấn luyện tiếp mô hình nền trên dữ liệu có nhãn của bài toán đích.\\
Huấn luyện đối kháng & Cách huấn luyện đưa nhiễu nhỏ vào gradient hoặc biểu diễn trung gian để mô hình bền vững hơn trước biến dạng đầu vào.\\
FGM & Fast Gradient Method, phương pháp tạo nhiễu đối kháng theo hướng gradient trên tầng embedding trong quá trình huấn luyện.\\
Occlusion & Phương pháp giải thích bằng cách che từng cửa sổ từ rồi đo mức thay đổi điểm dự đoán của mô hình.\\
Logit margin & Chênh lệch giữa logit của lớp HATE và logit của lớp CLEAN. Đại lượng này được dùng để tránh mất tín hiệu do xác suất bị bão hòa.\\
F1 & Trung bình điều hòa của Precision và Recall: $F1=2PR/(P+R)$.\\
ONNX Runtime Web & Bộ thực thi mô hình ONNX trong trình duyệt, cho phép suy luận phía trình khách mà không cần gửi văn bản lên máy chủ.\\
C0, C1, C2 & Ba điều kiện đánh giá lần lượt là câu sạch, câu né lọc và câu điểm mù.\\
\end{longtable}
\clearpage
\setcounter{table}{0}
...
Thư mục minh chứng cần được đội thi tạo và mở quyền truy cập trước khi nộp hồ sơ. Đường dẫn điền chính thức:
